...
#import "BulletStaticPlaneShape.h"
#import "BulletUpAxis.h"
#import "BulletCollision/CollisionShapes/btStaticPlaneShape.h"

@implementation BulletStaticPlaneShape
{
  btStaticPlaneShape *m_shape;
}

- (instancetype)initWithNormal:(vector_float3)normal constant:(float)constant
{
  self = [super init];
  if (self) {
    btVector3 btNormal(normal.x, normal.y, normal.z);

    m_shape = new btStaticPlaneShape(btNormal, constant);
